...
**已完成。** 实现帖子评论、楼中楼回复、cursor 分页、作者本人软删除、公开用户展示和最小审计事件。评论需要登录且 mock 实名状态为 `VERIFIED`。

本次不实现通知事件、举报审核、复杂折叠策略、富文本、图片上传或管理员删除。

## 第 6 次迭代：举报与审核后台

**当前迭代。** 先实现举报基础流：已实名用户可举报帖子和评论，服务端校验目标可见性、避免同一用户重复提交未处理举报，并记录 `REPORT_CREATED` 审计事件。

后续需要整体梳理后再进入审核后台：管理员 RBAC、审核队列、处置动作、用户举报、申诉和通知都属于更高风险的治理系统，不能在没有权限模型时硬编码。
